...
public class CustomerDashboardPanel extends JPanel {
        ...
        for (Event event : events) {
            ...
            if ("category".equals(event.getPricingType())) {
                priceDisplay = String.format("VVIP: RWF %.0f | VIP: RWF %.0f | Casual: RWF %.0f", 
                    event.getVvipPrice() != null ? event.getVvipPrice() : 0.0, 
                    event.getVipPrice() != null ? event.getVipPrice() : 0.0, 
                    event.getCasualPrice() != null ? event.getCasualPrice() : 0.0);
            } else {
    ...
    private void bookTickets() {
        ...
        if ("category".equals(event.getPricingType())) {
            ...
            JRadioButton vvipRadio = new JRadioButton("VVIP - RWF " + String.format("%.2f", event.getVvipPrice() != null ? event.getVvipPrice() : 0.0));
            JRadioButton vipRadio = new JRadioButton("VIP - RWF " + String.format("%.2f", event.getVipPrice() != null ? event.getVipPrice() : 0.0));
            JRadioButton casualRadio = new JRadioButton("Casual - RWF " + String.format("%.2f", event.getCasualPrice() != null ? event.getCasualPrice() : 0.0));
